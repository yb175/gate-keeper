...
export default function ChatWindow({
  ...
  const parseToolTrace = (content: string) => {
    if (
      content.startsWith("Call tool ") ||
      content.startsWith("Result: ") ||
      content.startsWith("Call parallel tools:")
    ) {
      return true;
    }
    try {
      ...
      if (
        ...
        ("result" in parsed || "isError" in parsed)
      ) {
        return true;
      }
    } catch (e) {}
    ...
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 min-h-[500px]">
        ...
        <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 space-y-4">
          {messages.length === 0 ? (
            ...
            messages.map((msg, index) => {
              ...
              const isTool =
                msg.role === "tool" || (!isUser && parseToolTrace(msg.content));

              if (isTool) {
                return (
                  <div
                    key={index}
                    className="flex flex-col space-y-1.5 p-3 rounded-sm bg-zinc-900/50 border border-zinc-800 font-mono text-xs max-w-full overflow-x-auto"
                  >
          ...
          {pendingApprovalId && (
            <div
              className={`border rounded-sm p-4 space-y-4 max-w-md ${
              ...
              <div className="flex items-start space-x-3">
                ...
                <div className="space-y-1">
                  ...
                  <p className="text-xs text-zinc-400">
                    ...
                    <code
                      className={`bg-zinc-900 px-1 py-0.5 rounded font-mono text-xs ${
                        pendingApprovalStatus === "APPROVED"
                          ? "text-green-400"
                          : pendingApprovalStatus === "REJECTED"
                            ? "text-red-400"
                    ...
                    >
                      {pendingToolName === "multiple_tool_calls"
                        ? "multiple parallel tools"
                        : pendingToolName || "unknown_tool"}
                    </code>
                    {pendingApprovalStatus === "APPROVED"
      ...
      <div className="lg:col-span-4 flex flex-col space-y-4">
        <div className="border border-zinc-800 bg-zinc-950 rounded-sm p-4 space-y-4">
          <h3 className="font-mono font-bold text-xs text-zinc-300 uppercase tracking-wider">
            Command Panel
          </h3>

          <div className="space-y-1.5">
            <label className="font-mono text-3xs text-zinc-500 uppercase block">
              Input Command
            </label>
            <textarea
              value={inputValue}
          ...
          >
            ...
            <span>
              {pendingApprovalId ? "Continue Conversation" : "Execute Prompt"}
            </span>
          </button>
        </div>

        <div className="border border-zinc-800 bg-zinc-950/50 rounded-sm p-4 space-y-2">
          <h4 className="font-mono text-3xs text-zinc-400 uppercase">
            Dashboard Tips
          </h4>
          <ul className="text-2xs text-zinc-500 space-y-1.5 list-disc pl-3">
            <li>Type any operation you want the agent to execute.</li>
            <li>
              If the tool requires review, the execution will pause and request
              your verification.
            </li>
            <li>
              You can review security rules in the <strong>Policies</strong>{" "}
              tab.
            </li>
          </ul>
        </div>
