...
export default function ApprovalTable({
  ...
  const handleAction = async (id: string, actionFn: (id: string) => Promise<void>) => {
    ...
    try {
      ...
      setError(err.response?.data?.error || err.message || "Failed to complete action");
    } finally {
      setActioningId(null);
    }
  };
  ...
  return (
    <div className="space-y-4">
      <div className="space-y-0.5">
        ...
        <p className="text-xs text-zinc-500">Track and respond to manual verification requests from your agent. Click a tool name to inspect parameters.</p>
      </div>
      ...
      {error && (
        <div className="p-3 bg-red-500/10 border border-red-500/20 rounded-sm text-red-400 font-mono text-xs flex items-center justify-between animate-fadeIn">
          ...
          <button onClick={() => setError(null)} className="text-red-500 hover:text-red-400 font-bold font-mono">✕</button>
        </div>
      )}
                    ...
                    <React.Fragment key={app.id}>
                      <tr className="hover:bg-zinc-900/30 transition-colors border-b border-zinc-900">
                        ...
                        <td className="px-4 py-3 text-zinc-200">
                          ...
                          >
                            <span className="text-zinc-500 font-normal text-3xs">{isExpanded ? "▼" : "▶"}</span>
                        ...
                        <tr className="bg-zinc-900/10 border-b border-zinc-900">
                          <td colSpan={5} className="px-4 py-3">
                            <div className="bg-zinc-950 p-3 rounded-sm border border-zinc-800 space-y-2">
                              <div className="font-bold text-zinc-500 text-2xs uppercase tracking-wider">Arguments Inspector</div>
                              {app.tool_name === "multiple_tool_calls" ? (
